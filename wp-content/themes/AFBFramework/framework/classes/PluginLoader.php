...
if ( ! class_exists( 'TB_Plugin_Activation' ) ) {
    /**
    ...
    class TB_Plugin_Activation {
         ...
         * @var string
         */
        public $menu = 'tbplugin-install-plugins';
         ...
         * does the tbplugin_init action hook, and hooks in the interactions to init.
         *
         * @since 1.0.0
         *
         * @see TB_Plugin_Activation::init()
         */
        public function __construct() {
            ...
            do_action_ref_array( 'tbplugin_init', array( $this ) );
         ...
         * @since 2.0.0
         *
         * @see TB_Plugin_Activation::admin_menu()
         * @see TB_Plugin_Activation::notices()
         * @see TB_Plugin_Activation::styles()
         */
        public function init() {

            // Load class strings.
            $this->strings = array(
                'page_title'                     => __( 'Install Required Plugins', 'tbplugin' ),
                'menu_title'                     => __( 'Install Plugins', 'tbplugin' ),
                'installing'                     => __( 'Installing Plugin: %s', 'tbplugin' ),
                'oops'                           => __( 'Something went wrong.', 'tbplugin' ),
                'notice_can_install_required'    => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'tbplugin' ),
                'notice_can_install_recommended' => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'tbplugin' ),
                'notice_cannot_install'          => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'tbplugin' ),
                'notice_can_activate_required'   => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'tbplugin' ),
                'notice_can_activate_recommended'=> _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'tbplugin' ),
                'notice_cannot_activate'         => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'tbplugin' ),
                'notice_ask_to_update'           => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'tbplugin' ),
                'notice_cannot_update'           => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'tbplugin' ),
                'install_link'                   => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'tbplugin' ),
                'activate_link'                  => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'tbplugin' ),
                'return'                         => __( 'Return to Required Plugins Installer', 'tbplugin' ),
                'dashboard'                      => __( 'Return to the dashboard', 'tbplugin' ),
                'plugin_activated'               => __( 'Plugin activated successfully.', 'tbplugin' ),
                'activated_successfully'         => __( 'The following plugin was activated successfully:', 'tbplugin' ),
                'complete'                       => __( 'All plugins installed and activated successfully. %1$s', 'tbplugin' ),
                'dismiss'                        => __( 'Dismiss this notice', 'tbplugin' ),
            );

            do_action( 'tbplugin_register' );
         ...
         *
         * We get the links in the admin notices to point to the tbplugin page, rather
         ...
         * @return null Returns early if not the tbplugin page.
         */
        ...
        public function thickbox() {
            ...
            if ( ! get_user_meta( get_current_user_id(), 'tbplugin_dismissed_notice_' . $this->id, true ) ) {
                add_thickbox();
            }
         ...
         *
         * @see TB_Plugin_Activation::init()
         * @see TB_Plugin_Activation::install_plugins_page()
         */
        ...
        public function install_plugins_page() {
            ...
            $plugin_table = new tbplugin_List_Table;

            // Return early if processing a plugin installation action.
            if ( isset( $_POST['action'] ) && 'tbplugin-bulk-install' == $_POST['action'] && $plugin_table->process_bulk_actions() || $this->do_plugin_install() ) {
                return;
            }

            ?>
            <div class="tbplugin wrap">

                <?php if ( version_compare( $this->wp_version, '3.8', '<' ) ) {
                    screen_icon( apply_filters( 'tbplugin_default_screen_icon', 'themes' ) );
                } ?>
                ...
                <form id="tbplugin-plugins" action="" method="post">
                    <input type="hidden" name="tbplugin-page" value="<?php echo $this->menu; ?>" />
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tbplugin-install'] ) && 'install-plugin' == $_GET['tbplugin-install'] ) ) {
                check_admin_referer( 'tbplugin-install' );
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            ...
                            'tbplugin-install' => 'install-plugin',
                        ),
                ...
                $fields = array( 'tbplugin-install' ); // Extra fields to pass to WP_Filesystem.
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'tbplugin' ) . '">' . __( 'Return to the Dashboard', 'tbplugin' ) . '</a>' ) . '</p>';
            ...
            elseif ( isset( $_GET['plugin'] ) && ( isset( $_GET['tbplugin-activate'] ) && 'activate-plugin' == $_GET['tbplugin-activate'] ) ) {
                check_admin_referer( 'tbplugin-activate', 'tbplugin-activate-nonce' );
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'tbplugin' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'tbplugin' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'tbplugin' ) . '</a>'  : '';

                // Define all of the action links.
                $action_links = apply_filters(
                    'tbplugin_notice_action_links',
                    array(
                        'install'  => ( current_user_can( 'install_plugins' ) )  ? $show_install_link  : '',
                        'activate' => ( current_user_can( 'activate_plugins' ) ) ? $show_activate_link : '',
                        'dismiss'  => $this->dismissable ? '<a class="dismiss-notice" href="' . add_query_arg( 'tbplugin-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . $this->strings['dismiss'] . '</a>' : '',
                    )
                );
                ...
                if ( ! empty( $this->strings['nag_type'] ) ) {
                    add_settings_error( 'tbplugin', 'tbplugin', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
                } else {
                    add_settings_error( 'tbplugin', 'tbplugin', $rendered, $nag_class );
                }
            }

            // Admin options pages already output settings_errors, so this is to avoid duplication.
            if ( 'options-general' !== $current_screen->parent_base ) {
                settings_errors( 'tbplugin' );
            }
        ...
        public function dismiss() {
            ...
            if ( isset( $_GET['tbplugin-dismiss'] ) ) {
                update_user_meta( get_current_user_id(), 'tbplugin_dismissed_notice_' . $this->id, 1 );
            }
        ...
        public function actions( $install_actions ) {
            ...
            // Remove action links on the tbplugin install page.
            if ( $this->is_tbplugin_page() ) {
                return false;
            }
        ...
        /**
         * Determine if we're on the tbplugin Install page.
         *
         * @since 2.1.0
         *
         * @return boolean True when on the tbplugin page, false otherwise.
         */
        protected function is_tbplugin_page() {
        ...
        public function update_dismiss() {
            ...
            delete_user_meta( get_current_user_id(), 'tbplugin_dismissed_notice_' . $this->id );
         ...
         * @since 2.4.0
         *
         * @return object The TB_Plugin_Activation object.
         */
        public static function get_instance() {

            if ( ! isset( self::$instance ) && ! ( self::$instance instanceof TB_Plugin_Activation ) ) {
                self::$instance = new TB_Plugin_Activation();
            }
    ...
    $tbplugin = TB_Plugin_Activation::get_instance();

}

if ( ! function_exists( 'tbplugin' ) ) {
    /**
     ...
     */
    function tbplugin( $plugins, $config = array() ) {

        foreach ( $plugins as $plugin ) {
            TB_Plugin_Activation::$instance->register( $plugin );
        }

        if ( $config ) {
            TB_Plugin_Activation::$instance->config( $config );
        }
...
if ( ! class_exists( 'tbplugin_List_Table' ) ) {
    /**
     ...
     */
    class tbplugin_List_Table extends WP_List_Table {

        /**
         * References parent constructor and sets defaults for class.
         *
         * The constructor also grabs a copy of $instance from the tbplugin class
         * and stores it in the global object TB_Plugin_Activation::$instance.
         *
         * @since 2.2.0
         */
        public function __construct() {
        ...
        protected function _gather_plugin_data() {
            ...
            TB_Plugin_Activation::$instance->admin_init();
            TB_Plugin_Activation::$instance->thickbox();
            ...
            foreach ( TB_Plugin_Activation::$instance->plugins as $plugin ) {
                if ( is_plugin_active( $plugin['file_path'] ) ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'tbplugin' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'tbplugin' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'tbplugin' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'tbplugin' ) : __( 'Recommended', 'tbplugin' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'tbplugin' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'tbplugin' ) );
                }
        ...
        protected function _get_plugin_data_from_name( $name, $data = 'slug' ) {
            ...
            foreach ( TB_Plugin_Activation::$instance->plugins as $plugin => $values ) {
                if ( $name == $values['name'] && isset( $values[$data] ) ) {
                    return $values[$data];
                }
            }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . __( 'Install', 'tbplugin' ) . ' %2$s">' . __( 'Install', 'tbplugin' ) . '</a>',
                        wp_nonce_url(
                            add_query_arg(
                                array(
                                    'page'          => TB_Plugin_Activation::$instance->menu,
                                    'plugin'        => $item['slug'],
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . __( 'Activate', 'tbplugin' ) . ' %2$s">' . __( 'Activate', 'tbplugin' ) . '</a>',
                        add_query_arg(
                            array(
                                'page'                 => TB_Plugin_Activation::$instance->menu,
                                'plugin'               => $item['slug'],
                                'plugin_name'          => $item['sanitized_plugin'],
                                'plugin_source'        => $item['url'],
                                'tbplugin-activate'       => 'activate-plugin',
                                'tbplugin-activate-nonce' => wp_create_nonce( 'tbplugin-activate' ),
                            ),
                            admin_url( 'themes.php' )
                        ),
                        $item['sanitized_plugin']
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'tbplugin' ), admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'tbplugin' ),
                'source' => __( 'Source', 'tbplugin' ),
                'type'   => __( 'Type', 'tbplugin' ),
                'status' => __( 'Status', 'tbplugin' )
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'tbplugin-bulk-install'  => __( 'Install', 'tbplugin' ),
                'tbplugin-bulk-activate' => __( 'Activate', 'tbplugin' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'tbplugin-bulk-install' === $this->current_action() ) {
                ...
                if ( empty( $plugin_installs ) ) {
                    echo '<div id="message" class="error"><p>' . __( 'No plugins are available to be installed at this time.', 'tbplugin' ) . '</p></div>';
                    return false;
                }
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            'page'          => TB_Plugin_Activation::$instance->menu,
                            'tbplugin-action'  => 'install-selected',
                ...
                // Loop through each plugin to install and try to grab information from WordPress API, if not create 'tbplugin-empty' scalar.
                $i = 0;
                foreach ( $plugin_installs as $plugin ) {
                    $api[$i] = plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) ? plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) : (object) $api[$i] = 'tbplugin-empty';
                    $i++;
                }

                if ( is_wp_error( $api ) ) {
                    wp_die( TB_Plugin_Activation::$instance->strings['oops'] . var_dump( $api ) );
                }
                ...
                $url   = add_query_arg( array( 'page' => TB_Plugin_Activation::$instance->menu ), admin_url( 'themes.php' ) );
                $nonce = 'bulk-plugins';
                $names = $plugin_names;
                ...
                echo '<div class="tbplugin wrap">';
                    if ( version_compare( TB_Plugin_Activation::$instance->wp_version, '3.8', '<' ) ) {
                        screen_icon( apply_filters( 'tbplugin_default_screen_icon', 'themes' ) );
                    }
            ...
            if ( 'tbplugin-bulk-activate' === $this->current_action() ) {
                check_admin_referer( 'bulk-' . $this->_args['plural'] );
                ...
                if ( empty( $plugins_to_activate ) ) {
                    echo '<div id="message" class="error"><p>' . __( 'No plugins are available to be activated at this time.', 'tbplugin' ) . '</p></div>';
                    return false;
                }
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s.</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'tbplugin' ), $imploded );
                }
 ...
 * @since 2.2.0
 */
add_action( 'admin_init', 'tbplugin_load_bulk_installer' );
function tbplugin_load_bulk_installer() {

    if ( ! class_exists( 'WP_Upgrader' ) && ( isset( $_GET['page'] ) && TB_Plugin_Activation::$instance->menu === $_GET['page'] ) ) {
        ...
        if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
            ...
            class TGM_Bulk_Installer extends WP_Upgrader {
                ...
                public function bulk_install( $packages ) {
                    ...
                    if ( TB_Plugin_Activation::$instance->is_automatic ) {
                 ...
                 * set to true for the tbplugin class.
                 *
                 * @since 2.2.0
                 *
                ...
                public function run( $options ) {
                    ...
                    if ( TB_Plugin_Activation::$instance->is_automatic ) {
                        ...
                        TB_Plugin_Activation::$instance->populate_file_path();
                ...
                public function install_strings() {
                    ...
                    $this->strings['no_package']          = __( 'Install package not available.', 'tbplugin' );
                    $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'tbplugin' );
                    $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'tbplugin' );
                    $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'tbplugin' );
                    $this->strings['process_failed']      = __( 'Plugin install failed.', 'tbplugin' );
                    $this->strings['process_success']     = __( 'Plugin installed successfully.', 'tbplugin' );
                ...
                public function activate_strings() {
                    ...
                    $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'tbplugin' );
                    $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'tbplugin' );
        ...
        if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
            ...
            class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                ...
                public function add_strings() {
                    ...
                    if ( TB_Plugin_Activation::$instance->is_automatic ) {
                        $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'tbplugin' );
                        $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'tbplugin' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'tbplugin' ) . '</span><span class="hidden">' . __( 'Hide Details', 'tbplugin' ) . '</span>.</a>';
                        $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'tbplugin' );
                        $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'tbplugin' );
                    }
                    // Default installation strings.
                    else {
                        $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'tbplugin' );
                        $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'tbplugin' );
                        $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'tbplugin' );
                        $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'tbplugin' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'tbplugin' ) . '</span><span class="hidden">' . __( 'Hide Details', 'tbplugin' ) . '</span>.</a>';
                        $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'tbplugin' );
                        $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'tbplugin' );
                    }
                ...
                public function bulk_footer() {
                    ...
                    foreach ( TB_Plugin_Activation::$instance->plugins as $plugin ) {
                        if ( ! is_plugin_active( $plugin['file_path'] ) ) {
                            echo '<p><a href="' . add_query_arg( 'page', TB_Plugin_Activation::$instance->menu, admin_url( 'themes.php' ) ) . '" title="' . esc_attr( TB_Plugin_Activation::$instance->strings['return'] ) . '" target="_parent">' . TB_Plugin_Activation::$instance->strings['return'] . '</a></p>';
                            $complete[] = $plugin;
                            break;
                        }
                        // Nothing to store.
                    ...
                    if ( empty( $complete ) ) {
                        echo '<p>' .  sprintf( TB_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'tbplugin' ) . '">' . __( 'Return to the Dashboard', 'tbplugin' ) . '</a>' ) . '</p>';
                        echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                    }
