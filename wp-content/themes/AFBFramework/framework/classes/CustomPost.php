...
class CP_Helper
    {
                                ...
                                function( $post, $data )
                                {
                                    ...
                                    if( ! empty( $custom_fields ) )
                                    {
                                        ...
                                        foreach ($custom_fields as $field) {
                                            ...
                                            $prefix = '_tb_'; //Underscore to keep it hidden from custom fields
                                            ...
                                            if(isset($field['metric'])){
                                                ...
                                                $metric = '';
                                                    ...
                                                    switch($field['type']) {
                                                        // text
                                                        case 'text':
                                                            echo '<input type="text" name="'.$field['id'].'" id="'.$field['id'].'" value="'.$meta.'" size="30" /> '.$metric.'
        ...
        public function add_custom_js_css()
        {
            ...
            wp_register_script('cp-helper', trailingslashit(FRAMEWORK_CLASSES).'js/cp-helper.js', array('jquery'), '1.0', true);
            wp_register_style('jquery-ui-custom', trailingslashit(FRAMEWORK_CLASSES).'css/jquery-ui-custom.css');
            
            wp_enqueue_script('jquery-ui-datepicker');
            wp_enqueue_script('jquery-ui-slider');
            wp_enqueue_script('cp-helper');
